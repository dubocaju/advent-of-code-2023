...
const findBestHandScore = hand => {
    if (!hand.includes('J')){
        return computeHandScore(hand);
    }
    
    let bestScore = 0;
    cards.forEach(card => {
        const currentScore = computeHandScore(hand.replaceAll('J', card));
...
const compareHands = (firstHand, secondHand) => {
    const firstHandScore = findBestHandScore(firstHand);
    const secondHandScore = findBestHandScore(secondHand);
